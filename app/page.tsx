...
export default function HomePage() {
  ...
  const movieMap = new Map<string, { group: ShowtimeGroup; cinemaCount: number }>();
  groups.forEach((g) => {
    const existing = movieMap.get(g.movie.id);
    if (existing) {
      existing.cinemaCount++;
    } else {
      movieMap.set(g.movie.id, { group: g, cinemaCount: 1 });
    }
  });
    ...
    <div className="min-h-screen">
      ...
      <section className="relative overflow-hidden border-b border-white/5">
        ...
        <div className="relative mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8 flex flex-col items-center text-center">
          ...
          <div className="grid grid-cols-3 gap-8 md:gap-16 border-t border-white/10 pt-8 px-8">
            ...
            <div className="flex flex-col items-center gap-1">
              <span className="text-2xl font-bold text-foreground">{uniqueMovies.length}</span>
          ...
          {isLoading ? (
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
              {filteredGroups.slice(0, 20).map((group) => (
                <MovieCard
                  key={`${group.movie.id}-${group.cinema.id}`}
                  movie={group.movie}
                  showtimes={group.showtimes}
                  cinemaName={group.cinema.name}
                />
              ))}
            </div>
          )}
